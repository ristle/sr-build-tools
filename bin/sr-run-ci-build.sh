...
export ubuntu_version=${ubuntu_version_name:-"bionic"}
export ros_release=${ros_release_name:-"melodic"}
...
"local") echo "Local run"
  ...
  docker run -w "$docker_user_home/sr-build-tools/ansible" -e LOCAL_USER_ID=$(id -u) $docker_flags --rm -v $HOME:/host:rw $docker_image  bash -c "&& git clone https://github.com/stonier/sophus.git /    tmp && cd /tmp &&  git checkout release/1.0.1-melodic && cmake . && make install && apt-get update && apt-get
   install -y vim git python-pip rsync && apt-get install -y ros-melodic-octomap* ros-melodic-pcl* ros-melodic-costmap-2d && git pull && git checkout $toolset_branch && git pull && PYTHONUNBUFFERED=1 ansible-playbook -v -i \"localhost,\" -c local docker_site.yml --tags \"local,$tags_list\" -e \"$extra_variables\" "
  ;;
